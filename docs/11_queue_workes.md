...
# Обновление 2026-02: надежность, мониторинг, новый формат Job

В систему очередей добавлены важные изменения по надежности и наблюдаемости:

1. Soft-timeout выполнения задач в воркере (`jobTimeoutSec`).
2. Retry с лимитом попыток и отправкой в DLQ (`<queue>.dlq`) после исчерпания лимита.
3. Детальный heartbeat по экземплярам воркеров (`workerInstanceId`), а не только по `workerId`.
4. Структурированное логирование действий воркера через PSR-логгер (с поддержкой Monolog).
5. Новый payload-native режим Job без лишней сериализации, при полной совместимости с legacy `JobInterface`.

Подробности и миграционный чеклист:

- [Обновление очередей: надежность и мониторинг](13_queue_reliability_update.md)
