...
- **M6 Primary OpenAPI = graph + escape hatch merge (§13)** — ✅ engine capability COMPLETE (Step 8): the primary producer is switchable via `OpenApiSource` (Legacy default = byte-compat swagger-php; Metadata opt-in = graph + narrow escape hatch, validated, two forbidden-content guards + provenance + external-ref policy + single-source fingerprint). F+N defaults stay `Legacy`; N production activation (flipping `SPSFW_OPENAPI_SOURCE=metadata`) is **deferred to M7/M8** after Step 9 removes controller OA and the divergence backlog closes.
...
### Шаг 4. OpenApiEmitter + StandardErrorPolicy + parity (F; depends 3) — M3 — ✅ ВЫПОЛНЕН
...
- **Статус:** завершён. `OpenApiEmitter` (array-first, OpenAPI 3.1.0, cycle-safe schema collection), `StandardErrorPolicy` (400/401/403/429/500, no 422), `ParityReport` (round-trip parse + normalize + compare) — `OpenApiEmitterTest`/`ParityReportTest` зелёные. N-probe (`gen_n_openapi_parity.php`): 387 ops → 333 paths / 151 schemas в parity-режиме (393 migration-gaps НЕ блокируют; strict/managed HALT через `throwOnErrorsAndWarnings()`); publication **blocked** 15 структурными fatals (14 duplicate METHOD:path + 1 schema-name collision); normalized parity baseline (после round-trip `dump → parse`) — **5002 divergences** (фронт M7–M9, см. AUDIT §4.11). Bug sentinel-протечки swagger-php (`Generator::UNDEFINED` в OA items-fallback) найден и закрыт regression-тестом. `composer test` 27/27. Primary `openapi.yml`/preload/production-flow нетронуты.
- **Решения (Step 4, зафиксированы ДО начала emitter-работы — реализованы):**
  1. **`symfony/yaml` — прямая dependency `composer.json`.** YAML-сериализация идёт через `Symfony\Component\Yaml\Yaml`, а **НЕ** hand-rolled emitter (хрупкость quoting/escaping/multiline-flow → silent spec corruption). `symfony/yaml` объявляется **прямой** зависимостью SpsFW (`require`), а **не** транзитивной через `zircote/swagger-php` — swagger-php может быть удалён/понижен (M8/§13 OA-escape-hatch-scope сужается), и тогда transitively-pulled `symfony/yaml` исчезнет из lockfile. Прямая dep гарантирует, что compile-engine owns свою YAML-связь явно.
  2. **Array-first.** Emitter сначала строит **детерминированный PHP-массив** (полное AST OpenAPI-документа: `openapi/info/paths/components/securitySchemes/...`, ключи отсортированы детерминированно, коллекции в стабильном порядке = discovery+natural-order). YAML — **только финальная сериализация** этого массива (`Yaml::dump($array, ...)`). Это разделяет «логика spec-конструирования» (тестируется на массиве, `assert_same`) и «формат» (функция `dump`). Никаких строковых конкатенаций YAML, никаких отступов вручную.
  3. **Parity после round-trip parse/normalization, а не по тексту/пробелам.** Сравнение generated-spec vs legacy `openapi.yml` (swagger-php) делается **не** побайтно и не по строкам: оба YAML'а парсятся обратно в массив (`Yaml::parseFile`), **нормализуются** (сортировка ключей, canonical-форма refs/scalar-коercion, drop ключей с default-значениями, fold известных swagger-php-specific noise) и сравниваются **массив-к-массиву** (`===`). Пробелы/отступы/quoting-style/порядок ключей в тексте — **не** часть parity-контракта (swagger-php и symfony/yaml форматируют по-разному). Фиксированные documented-divergences (известные отличия новой проекции от legacy) перечислены явно и исключаются из diff.
- **Тесты/команды:** snapshot массива (до YAML); parity-diff vs `openapi.yml` **после round-trip parse/нормализации** (а не текстовый diff); фиксация divergence через `#[Items]/#[Field]/#[Response]`.
- **Критерии:** для мигрированных endpoint-ов normalized(generated) ≡ normalized(legacy) (с точностью до documented fixed-divergences); `symfony/yaml` — прямая dependency; emitter не содержит hand-rolled YAML-форматирования.

### Шаг 5. Coordinator (engine) + staging/publication API + managed mode (F; depends 3,4) — ✅ ВЫПОЛНЕН
...
- **Статус:** завершён. Реализовано: `ApplicationContext` (явный context; **декаплинг** ownership mode `legacy|managed` и diagnostic policy `parity|strict` — независимые оси, `warningsBlock()` только в strict); `DICacheBuilder::compileOnly()` — строит и возвращает DI-map + job registry **без** записи в prod-cache и **без** `setCompiledMap()` на singleton (legacy `compile()` → BC-wrapper, теперь требует container и кидает без него); `Publication\Fingerprinter` (детерминированный md5 fingerprint над compiler-version + sorted source-file md5 + canonicalized config; `built_at` **исключён** из fingerprint, но в manifest); `Publication\StagingPublisher` (`flock(.cache/.compile.lock)` → staging на той же ФС → **backup+journal ДО** рискованного rename → per-file atomic rename → rollback journal восстанавливает старый комплект при сбое на любом шаге → cleanup; **нет** pre-unlink; manifest публикуется **последним**); `Coordinator` (один flow: route metadata/cache + DI/job + secondary OpenAPI + validation + manifest; **все** diagnostics агрегируются в один `CompileDiagnostics` — op-compiler + route-compiler + emitter + `OpenApiValidator` + DI; gate `!errors && !(strict && warnings)`); `CompileResult` VO (`published/dryRun/reason/fingerprint/...`); dev-only `DevCompileRunner` + `bin/spsfw-compile.php` (thin wrapper, без своей логики сборки). **8 обязательных тестов** покрыты: `StagingPublisherTest` (success + rollback на КАЖДОМ шаге via fault-injection + rollback newly-created + manifest-last + lock-contention), `CoordinatorFlowTest` (clean publish + ERROR→старые хеши неизменны + warning parity-проходит/strict-блокирует + mode↔policy декаплинг + dry-run), `FingerprinterTest` (determinism + built_at-excluded + content/config change + key-order-independent), `DICacheBuilderCompileOnlyTest` (no prod write + singleton не мутируется behavioral-доказательством + robust), `DevCompileRunnerTest`. `composer test` 32/32.
- **N dry-run (probe `gen_n_coordinator_dryrun.php`, read-only):** на реальном дереве N — `published=false`, `reason='dry-run'`, текущие `.cache`-артефакты **не изменены** (md5 до===после). Coordinator агрегирует **37 ERROR-записей** (route-compiler + emitter + operationId-resolver, по записи на операцию) — §4.11 baseline «15» это их **подмножество**. 12 distinct структурных проблем зафиксированы как обязательный worklist перед Step 6b (см. AUDIT §4.12): 7 duplicate `METHOD:path` (6 Core↔Next auth overrides + 1 intra-controller `EmployeeDocumentsController`), 4 operationId-коллизии (auth overrides), 1 `CreateNewsDto` schema-name collision. **Текущий N нельзя в managed production, пока они есть; ни один ERROR не понижен до warning.** Шаг 6 не начат.
...
### Шаг 8. Primary OpenAPI = graph + escape hatch (F; depends 4,6a) — M6 — ✅ engine capability complete; N production activation pending M7/M8
- **Status (2026-07):** engine capability COMPLETE — the PRIMARY `.cache/swagger/openapi.yml` producer is switchable behind a new 4th independent `OpenApiSource` axis (`Legacy` default = byte-compat swagger-php; `Metadata` opt-in = graph doc merged with the narrow escape hatch, validated). **F default stays `Legacy`; N default stays `Legacy`** — the `Metadata` path is exercised ONLY by the dev/probe throwaway run. The N production flip to `Metadata` is **deferred to M7/M8** (after Step 9 removes controller OA + the divergence backlog closes). This is NOT a completed production flip.
- **Modify:** `OpenApiSource` (enum, mirrors `RuleSource`) + `OpenApiEscapeHatch` (VO) are first-class `ApplicationContext` axes; `Coordinator` builds the graph ONCE (`OpenApiEmitter::emit()`), then under `Metadata` merges the escape hatch (`OpenApiEscapeHatchMerger`) into a copy and re-validates; under `Legacy` it keeps `DocsUtil::produceLegacyOpenApiYaml()` byte-identical (and SKIPS the swagger-php scan under `Metadata`). The SECONDARY `openapi.generated.yml` stays the pure graph under BOTH modes. `recordedConfig()` records `openapi_source`; `Fingerprinter` carries the escape hatch via ONE canonical representation (`escapeHatchCanonical` → both the fingerprint payload and the manifest `escape_hatch_hash`); `COMPILER_VERSION` bumped `spsfw-compile-3` → `spsfw-compile-4`.
- **Two forbidden-content guards (mandatory):** (a) an **annotation-level allowlist guard** (`EscapeHatchAnalysisGuard`, a swagger-php processor) inspects the RAW `Analysis` — only `Schema/Property/Items/Discriminator/AdditionalProperties/ExternalDocumentation/Xml` (+ root `OpenApi`) allowed; ANY operation/path/parameter/request-body/response/security annotation → FATAL naming file/class + type (catches `#[OA\Get]` even though the schema-preserving pipeline drops `BuildPaths`); (b) an **exact-shape guard** on the generated partial — only `openapi/info/components.schemas`; `paths` must be empty; ANY other root section or `components` key other than `schemas` → FATAL.
- **Provenance (no silent last-wins):** scan targets resolve via `OpenApiEscapeHatch::resolveScanFile()` (class-string FQCN → reflection `getFileName()`; else project-relative path; `is_file` check); resolved files are deduplicated (same file via FQCN and path scans once), deterministically sorted; the SAME sorted list feeds scan + fingerprint; each file is scanned separately for provenance; a duplicate fragment schema key across two targets → FATAL; a requested key missing → FATAL; a graph↔fragment collision → FATAL (graph wins).
- **External-ref policy:** `OpenApiValidator` treats external URL/file `$ref` as resolved, so the merger forbids them explicitly — any `$ref` NOT starting with `#/` → FATAL. Only local `#/components/schemas/...` refs are allowed.
- **Single fingerprint source of truth:** `OpenApiSource` reaches the fingerprint via `recordedConfig()` → `config` (NOT passed a second time); the escape hatch has ONE canonical representation (`class:<FQCN>` / `file:<project-relative-path>` keys + content `md5`; no absolute deploy paths — a file outside projectRoot is the `out-of-root` sentinel, already FATALed by the merger upstream); the manifest carries ONLY the `escape_hatch_hash` md5 (no targets, no paths, no content).
- **Emit-once:** the graph is built by a SINGLE `emit()` call (Coordinator code review — one call site); the array-first contract is pinned by focused tests (merge/validate/dump on a built doc add no diagnostics), NOT by a diagnostic count (CompileDiagnostics dedups).
- **Тесты (F):** characterization-FIRST (`EscapeHatchPipelineCharacterizationTest` — polymorphic survival + forbidden-annotation detection in isolation); `OpenApiEscapeHatchMergerTest` (11 cases — empty no-op, whitelist extraction, missing/collision/forbidden/dup-key FATALs, same-file dedup, refs resolve, external-ref FATAL, determinism, no x-fqcn/nullable); `OpenApiSourceTest`; `OpenApiPrimarySourceTest` (Coordinator-level — Legacy byte-identical to DocsUtil, Metadata == dump(merge(emit())), secondary pure graph both modes, ERROR→not published, manifest openapi_source+escape_hatch_hash, fingerprint differs on flip); extended `FingerprinterTest` + `OpenApiEmitterTest` (emit-once contract). All green (46 files).
- **N probe:** `docs/metadata_compiler_audit/gen_n_openapi_source_parity.php` — two FULL-Coordinator runs (Legacy/Metadata) on real N controllers: A=0/B=0 errors, route/DI/job/secondary byte-identical, A reproduces the legacy producer, B==pure graph + passes OpenApiValidator, fingerprints differ, real N/.cache untouched. Divergence (A 301 paths/348 ops/341 schemas vs B 333/380/151) is recorded as M7–M9 backlog.
- **Критерии:** spec валиден; escape hatch ограничен (two guards); расхождения детерминированы. ✅ met for the engine; N production activation is M7/M8.

### M8b. Route-first / inference-first operation contract (F engine + миграция F/N; перед Шагом 9)
- **Решение:** `#[Route]` становится каноническим источником operation-контракта. К существующим positional `path`+`httpMethods` (runtime Router — без BC-разрыва) добавлены опциональные named-only поля: `summary, description, operationId, tags, deprecated, documented, returns, successStatus, errors`. Успех выводится по приоритету: `Route::returns` → legacy `#[ApiResponse]` → нативный return-type → консервативный AST-анализ `Response::json(...)` → diagnostic. Коллекции — через `returns: [Dto::class]` (без `returnsMany`). Стандартные 400/401/403/429/500 — по-прежнему от `StandardErrorPolicy` (без изменений). Редкие endpoint-специфичные коды приходят **только** из `Route::errors` или явного `#[ApiResponse]` — не из тела метода. **AST-вывод error-статусов убран (fix-pass D6):** литеральные/динамические `Response::error(...)` и нетрасируемые error-пути больше не порождают статусов и не синтезируют OpenAPI `default`; `default`-ответ не эмиттится вовсе (`StandardErrorPolicy` описывает стандартный 5xx, нестандартные — только явная разметка).
- **AST-анализ** (`src/Core/Compile/Introspection/ResponseAstAnalyzer.php`) — на nikic/php-parser v5 (теперь прямая dependency `^5.0`), parse-once + memoize по файлу, name-resolve через `NodeVisitor\NameResolver`; тело метода находится по файлу + FQCN/trait + Reflection start/end lines (не по короткому имени); случаи A–F; любое неоднозначное ⇒ `definite:false`, никогда не падает на parse-failure. Контроль противоречий: определённый native/AST-вывод, противоречащий `Route::returns` ⇒ compile ERROR (не silent override); неоднозначные случаи (`mixed`/`array`/`JsonSerializable`) `returns` свободно переопределяет. **Native-union (fix-pass D2):** `Dto|Response` и `Dto|Response|null` разрешаются в DTO (`SpsFW\Core\Http\Response` и `null` — transport/no-body-ветви, исключаются; ровно один оставшийся schema-тип ⇒ definite); два разных DTO/schema-типа в union ⇒ неоднозначно (non-definite). **Schema и status разделены (fix-pass D3):** schema-приоритет `returns`→ApiResponse→native→AST; статус — явный `successStatus`, иначе единственный однозначный ApiResponse/AST-статус, иначе 200; разные статусы у success-ветвей с одинаковой схемой не схлопываются в последнюю (диагностика, fallback 200). **204+body (fix-pass D4):** тело успеха при финальном статусе 204 — compile ERROR независимо от источника (returns/native/AST/ApiResponse). AST-анализирует **только** success-ветви; error-статусы из тела не выводятся (см. D6 выше).
- **`#[Operation]`** остаётся BC-фолбэк-атрибутом (читается только если поле Route не задано); `documented:false` заменяет `exclude:true`; конфликт Route↔Operation ⇒ diagnostic. Явный `#[ApiResponse]` больше не замещает inference целиком — error/complex-ответы **дополняют** выведенный успех; та же status + разная schema ⇒ ERROR.
- **Lifecycle миграции (throwaway codemod, НЕ Step 9):** `Operation` → поля Route (**новых `#[Operation]` не появляется** — это BC-fallback, который codemod убирает, а не вводит); простой success `ApiResponse(schema:X, collection:b, status:N)` → `returns: X|[X]` + `successStatus:N` **только при lossless-замене** — если при этом теряется status / schema / cardinality / content-type / description, `#[ApiResponse]` остаётся; стандартные 400/401/403/429/500-атрибуты удаляются (их даёт policy); нестандартные простые ошибки → `Route::errors`; редкие complex-ответы (несколько success-схем, headers, file/stream, HTML, redirect, non-JSON, полиморфизм, **а также success-ApiResponse с осмысленным description**) остаются как `#[ApiResponse]`. **Массовая миграция ~119 success-`#[ApiResponse]` с русскими description НЕ планируется** — они остаются legacy-`#[ApiResponse]` (подаются как supplement к выводенному успеху), rollback-слой до Шага 9. **Legacy swagger-php `#[OA\…]]` НЕ трогается** — это rollback до Шага 9.
- **Acceptance:** Orval-contract equivalence (METHOD:path, operationId, params, request/response schemas, cardinality, security, известные error-статусы) — НЕ byte-identical YAML. Жёсткий гейт сохранения контракта: множество операций METHOD:path byte-identical с baseline (M8b переносит response-схемы; нестандартные error-коды — только из `Route::errors`/явного `#[ApiResponse]`; **fix-pass D6 убрал синтезированные `default`-ответы** — `default` не эмиттится, операции не добавляются/не удаляются/не исключаются); operationId остаётся lockfile-derived; Orval-diff без удалённых/переименованных функций — только перенесённые success-схемы.

### Шаг 9. Clean controllers from OA (репо N; depends 8) — M7
- **Действия:** удалить `#[OA\Get/Post/Patch/Parameter/RequestBody/Response/Security]` с мигрированных контроллеров `next`; operation-поля (summary/tags/deprecated/...) — на `#[Route]`, тело/схема — на `#[Response]/#[Items]/#[Field]`. **`#[Operation]` не возвращается** (после M8b это BC-fallback, который codemod уже убрал; Шаг 9 новых `#[Operation]` не вводит). DTO OA пока остаётся.
