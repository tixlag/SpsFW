...
SQL);
            $this->execute(
                'CREATE UNIQUE INDEX queue_outbox_deduplication_uidx ON queue_outbox (deduplication_key)'
            );
            $this->execute(
                'CREATE INDEX queue_outbox_due_idx ON queue_outbox (available_at, next_attempt_at, claimed_until)'
            );
            return;
        }
...
SQL);
    ...
    {
        if ($this->getAdapter()->getAdapterType() === 'pgsql') {
            $this->execute('DROP INDEX IF EXISTS queue_outbox_due_idx');
            $this->execute('DROP INDEX IF EXISTS queue_outbox_deduplication_uidx');
            $this->execute(<<<'SQL'
ALTER TABLE queue_outbox
