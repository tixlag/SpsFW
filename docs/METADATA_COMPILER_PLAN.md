...
- **Lifecycle миграции (throwaway codemod, НЕ Step 9):** `Operation` → поля Route; простой success `ApiResponse(schema:X, collection:b, status:N)` → `returns: X|[X]` + `successStatus:N`; стандартные 400/401/403/429/500-атрибуты удаляются (их даёт policy); нестандартные простые ошибки → `Route::errors`; редкие complex-ответы (несколько success-схем, headers, file/stream, HTML, redirect, non-JSON, полиморфизм) остаются как `#[ApiResponse]`. **Legacy swagger-php `#[OA\…]]` НЕ трогается** — это rollback до Шага 9.
- **Acceptance:** Orval-contract equivalence (METHOD:path, operationId, params, request/response schemas, cardinality, security, известные error-статусы, default ErrorResponse) — НЕ byte-identical YAML. Жёсткий гейт сохранения контракта: множество операций METHOD:path byte-identical с baseline (M8b переносит response-схемы и добавляет классифицированные error-коды/`default` — не добавляет/не удаляет/не исключает операции); operationId остаётся lockfile-derived; Orval-diff без удалённых/переименованных функций — только перенесённые success-схемы и добавленные error-типы.
