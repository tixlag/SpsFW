...
Для атомарной публикации сообщения вместе с бизнес-записью см. `16_outbox_pattern.md` и `developers/queue-outbox.md`. Этот документ описывает runtime воркеров и DLQ, а не сам transactional outbox.
...
7. Добавлен transactional outbox для надёжной публикации сообщений вместе с БД-транзакцией и отдельный relay loop для их доставки после commit.
